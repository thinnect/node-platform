...
#else
#ifdef CRC32_AVR_PROGMEM
#include <avr/pgmspace.h>
const uint32_t PROGMEM crc_tab[] = {
#else
uint32_t crc_tab[] = {
#endif//CRC32_AVR_PROGMEM
...
#ifdef CRC32_AVR_PROGMEM
	return(pgm_read_dword_near(&crc_tab[(crc ^ data) & 0xFF]) ^ (crc >> 8));
#else
	return(crc_tab[(crc ^ data) & 0xFF] ^ (crc >> 8));
#endif//#CRC32_AVR_PROGMEM
}
...
uint32_t crc32_block(uint32_t crc, const void *data, uint32_t len){
	uint32_t i;
	for(i = 0; i < len; i++){
		crc = crc32_byte(crc, ((uint8_t *)data)[i]);
	}
	return(crc);
}
