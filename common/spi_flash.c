...
#define FLASH_SIZE_32MBIT ((32UL / 8UL) * 1024UL * 1024UL)
#define FLASH_SIZE_64MBIT ((64UL / 8UL) * 1024UL * 1024UL)
...
bool spi_flash_init(uint32_t flash_size)
{
    ...
    else if (FLASH_SIZE_32MBIT == flash_size)
    {
        debug1("32Mbit flash");
        // bootloader
        spi_flash_partitions[0].start = 0x0;
        ...
        // application filesystem
        spi_flash_partitions[2].start = 0x100000;
        spi_flash_partitions[2].end = 0x400000;
    }
    else
    {
        for (uint8_t i = 0; i < 10; i++)
